...
export default function ExtractedHistory({
  currentPalette,
  history,
  onSaveToHistory,
  onLoadFromHistory,
  onDeleteHistory,
  onClearHistory
}: ExtractedHistoryProps) {

  // 現在の抽出パレットが完全に空かどうかを判定
    ...
    <div className="mt-12 pt-8 border-t border-gray-100">
      ...
      {history.length === 0 ? (
        <div className="bg-gray-50 border border-gray-200 border-dashed rounded-3xl p-10 text-center text-gray-400">
          ...
          <p className="font-bold">抽出した色の履歴はありません</p>
          <p className="text-sm mt-1">グラデーションから抽出した色セットを「履歴に保存」できます。</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {history.map((pal, index) => (
            <div key={index} className="bg-white border border-gray-200 p-4 rounded-2xl shadow-sm hover:shadow-md transition-shadow flex flex-col gap-3 group">
              <div className="flex h-10 rounded-lg overflow-hidden border border-gray-100">
                {/* 8色の枠をループして表示 */}
              ...
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs font-bold text-gray-400">#Extracted-{history.length - index}</span>
